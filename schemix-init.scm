...
;;;;; I/O (Schemix has no ports)

(define (input-output-port? p) #f)
(define (close-port p) #f)
(define (call-with-input-file s p) #f)
(define (call-with-output-file s p) #f)
(define (with-input-from-file s p) #f)
(define (with-output-to-file s p) #f)
(define (with-input-output-from-to-files si so p) #f)
